...
- For larger datasets and projects it would be ideal to try to match primary keys across multiple tables to allow for joins and more complex analyses. Creating indexes to speed up queries would also be important to consider, but I did not find it necessary here. Furthermore, splitting the `database.py` file into more helper files could help with modularity and organization, especially if multiple pages or complex workflows were to be added. 
- The current framework supports the addition of more complex filters with the dropdown query system. In larger scale datasets, it would be a good idea to take advantage of this to isolate groups of individuals with similar characteristics (or characteristics of interest) to better refine treatments or therapies. K-means clustering, for example, can be used to identify and categorize these groups. This could also be coupled with machine learning models to predict treatment response or cell population frequencies. 
